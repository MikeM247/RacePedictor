...
export function ActivityRecordContent({ activity, onClose, parentLabel = "Back to Training", headingId = "activity-detail-heading", disclosures, onDisclosureChange }: { activity: ActivityDetail; onClose?: () => void; parentLabel?: string; headingId?: string; disclosures?: DisclosureState; onDisclosureChange?: (name: keyof DisclosureState, open: boolean) => void }) {
  ...
  return <><div className="detail-header">{onClose ? <DetailBackButton onClose={onClose} label={parentLabel} /> : null}<div className="detail-header-meta"><p className="eyebrow">{sportLabel(activity.sport)}</p><span className="detail-record-label">Activity record</span></div><h2 id={headingId} tabIndex={-1}>{activity.title || sportLabel(activity.sport)}</h2><p>{formatActivityDate(activity.localOccurredAt, activity.occurredAt)}</p></div><section className="detail-core-readout" aria-labelledby={coreReadoutHeadingId}><div className="detail-section-heading"><div><p className="eyebrow">Session summary</p><h3 id={coreReadoutHeadingId}>Run at a glance</h3></div></div><dl className="detail-metrics detail-primary-metrics">{coreMetrics.map(([label, value], index) => <div key={label} className="detail-metric"><dt><span className="detail-metric-index" aria-hidden="true">0{index + 1}</span>{label}</dt><dd>{value}</dd></div>)}</dl></section><ActivityCoachReview activityId={activity.id} />
