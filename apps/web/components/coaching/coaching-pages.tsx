...
export function PlanPage({ onlineMode = false }: { onlineMode?: boolean }) {
  ...
  async function copyCodexInstructions() {
    ...
    const instructions = `Provide Codex with the validated coaching-context.v1.json for ${artifactId}, the active approved plan, and the selected Second Brain context. Ask for one coaching-plan-proposal.v2 JSON file compatible with that context, goal, and routine. Require milestones to contain every supported intermediate race target, with no values inferred from uncertain prose. Return to Plan, choose Import proposal, select that JSON, import it explicitly, review the goal and milestone relationship, then explicitly confirm a decision. Publishing, copying, downloading, importing, and leaving Plan never activate a plan.`;
  ...
  return <CoachShell page="plan" title="Plan" subtitle={onlineMode ? "Choose which explicitly approved structured plan is active" : "Set your goal and explicitly approve each plan version"} meta={planLoadState === "error" ? "Plan unavailable" : planLoadState === "loading" ? "Loading plan…" : activePlan ? `Active · ${activeContentVersion ?? `record ${String(activePlan.version ?? 1)}`}` : "No active plan"}>
    ...
    {workflowStage === "continue" ? <section className="coach-panel" aria-labelledby="codex-continuation-heading">
      ...
      <p>Open Codex on this computer and provide the validated <strong>coaching-context.v1.json</strong>, the active approved plan, and the selected Second Brain context. Ask Codex to return one <strong>coaching-plan-proposal.v2</strong> JSON file with every supported intermediate race target in <strong>milestones</strong>. Return here, choose Import proposal, select that JSON, explicitly import it, review the goal and milestone relationship, then explicitly confirm a decision. Publishing, copying, downloading, importing, and leaving do not activate a plan.</p>
