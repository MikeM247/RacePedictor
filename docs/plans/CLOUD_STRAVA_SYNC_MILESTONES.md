...
| M8 | Free-tier services are provisioned, secrets configured, and production is smoke-tested. | Core 82/82, database 54/54, web 77/77, typecheck, production build, focused manual-backfill and route-security coverage, Prisma, audit, and diff gates pass. History imports reserve provider capacity before I/O and retain durable checkpoints on a pause or retry. | Owner login, Neon migration, private R2 write path, real Strava OAuth, bounded real-history import, and the deployed pacing smoke all pass. The imported workout remains visible online. Live automatic-webhook, independent raw-integrity read, and local selected-context evidence remain. | In progress — final live journeys |
