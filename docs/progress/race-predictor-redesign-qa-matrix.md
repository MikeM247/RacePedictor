...
|---|---:|---:|---:|---:|---:|
| Dashboard | 0 | 5 | 7 | 0 | 0 |
| Activity upload/import | 1 | 11 | 0 | 0 | 0 |
| Race goal setup | 1 | 10 | 0 | 0 | 0 |
| Prediction review | 1 | 4 | 6 | 0 | 0 |
| Training trends | 6 | 2 | 2 | 0 | 0 |
| Data confidence | 1 | 9 | 0 | 0 | 0 |
| Mobile navigation | 2 | 5 | 3 | 0 | 0 |
| Deployment smoke test | 10 | 1 | 0 | 0 | 0 |
...
| DB-A01 | Accessibility | Keyboard and screen-reader pass through shell, three groups, links, and status content | Skip link, one H1, logical headings, labeled regions, visible focus, text status, and no color-only meaning | P0 | IN PROGRESS | 2026-09-24 F05 initial-route and expanded-disclosure axe scans pass on all six routes in local and online Chromium fixtures; skip-link focus and named navigation pass. Screen reader, switch control and unfamiliar-runner comprehension remain outstanding. |
...
| AI-A01 | Accessibility | Keyboard file selection path, validation, result review, and Strava return path | Labels/errors are associated; first invalid field receives focus; status changes are announced once and meaningfully | P0 | IN PROGRESS | 2026-09-24 route-level axe scans pass on Data Quality and all expanded disclosures; F08 import fixtures pass in local/online runs. Full keyboard validation, live announcements and provider-return assistive-technology checks remain. |
...
| PR-A01 | Accessibility | Keyboard/screen-reader detail, disclosure, chart alternative, and Back behavior | Heading structure, labeled controls, visible focus, semantic text alternative, and restored launcher focus | P0 | IN PROGRESS | 2026-09-24 initial and expanded detail/disclosure axe scans pass in local/online Chromium; Training Back/focus restoration passes. Manual screen-reader and assistive-technology review remains open. |
...
| TT-A01 | Accessibility | Read chart via text equivalent and navigate controls with keyboard | Equivalent summary conveys all essential chart information; controls and selected filters are labeled | P0 | IN PROGRESS | 2026-09-24 all-route and expanded-disclosure axe scans pass on the local and online Training route. Manual keyboard navigation and assistive-technology review remain. |
...
| DC-A01 | Accessibility | Navigate data issue, status, recovery, and return flow with keyboard and screen reader | Status is textual, contrast meets 4.5:1/3:1 rules, focus and announcements are meaningful and non-repeating | P0 | IN PROGRESS | 2026-09-24 Data Quality route axe scan passes in local/online Chromium. Recovery keyboard/announcement, contrast measurement and screen-reader checks remain. |
...
| MN-A01 | Accessibility | Keyboard, switch-control, screen-reader, 44px target, 200% zoom, reduced-motion, and forced-colors checks | All workflows operate by keyboard; focus visible; targets meet 44×44; essential meaning is not color-only | P0 | IN PROGRESS | 2026-09-24 initial and expanded-disclosure axe scans pass on all six routes; Plan activation, Calendar detail/amendment dialogs pass axe; reduced-motion and forced-colors checks pass. Browser-level 200% zoom, screen reader, switch control, software keyboard and remaining active-workflow resize cases are outstanding. Earlier axe failures are superseded, not erased. |
...
| MN-PW01 | Playwright candidate | Run core journey at 320px and 768px with keyboard and pointer | Shell, detail replacement, Back restoration, dialogs, and no-horizontal-overflow assertions pass | P0 | IN PROGRESS | 2026-09-24 local/online Chromium suites pass at 320/767/768/1199/1200px and surrounding widths, including 390px. Earlier “200%” overflow checks were viewport proxies, not browser zoom. Full journey by keyboard and active-state resizing remain open. |
...
| DS-A01 | Accessibility | Run route smoke with keyboard and axe/equivalent if present | Skip link, one H1, labeled nav, current destination, visible focus, contrast, and semantic statuses pass | P0 | IN PROGRESS | 2026-09-24 Settings route and expanded preference/privacy/reminder disclosures pass axe in local and online Chromium; manual keyboard, contrast and assistive-technology checks remain. |
...
**F10 decision (2026-09-16): BLOCKED at that checkpoint.** The complete online-fixture suite reproduced a focus-return defect after Plan conflict recovery and heading-order violations on redesigned routes. The product defects were repaired and their final regressions pass in the 2026-09-24 evidence below. Browser-level 200% zoom, assistive technology, human comprehension, live-provider, authenticated deployed and deployment checks remain outstanding.
...
## F10 release-readiness update — 2026-09-24

The 2026-09-16 product failures and the 2026-09-23 aggregate-shutdown failure are superseded by the final evidence below. The 87-row progress totals were recomputed after recording partial automated accessibility coverage: Dashboard 0 TODO / 5 in progress / 7 pass; Import 1/11/0; Goal setup 1/10/0; Prediction 1/4/6; Training trends 6/2/2; Data confidence 1/9/0; Mobile 2/5/3; Deployment 10/1/0. No row is marked complete solely from automated axe.

| Exact command | Mode and fixture | Result | Artifact |
|---|---|---|---|
| `npm run test:e2e:local --workspace @racepredictor/web` | Windows; Chromium; one worker; sequential isolated general-local SQLite/snapshot/vault/exchange, dedicated seeded Home and dedicated seeded readiness; responsive cases include 320, 375, 390, 414, 767, 768, 1024, 1199, 1200 and 1440 CSS px. Ran with local process rights that permit Playwright's Next servers to shut down. | General 32/32, Home 16/16, readiness 5/5; aggregate exit 0; 0 failures, flaky attempts or skips. | `.local/e2e/digital-coach-1790232293317-20564/results/local.json`; `.local/e2e/digital-coach-1790232594622-4140/results/home-local.json`; `.local/e2e/digital-coach-1790232664574-50012/results/readiness-local.json` |
| `$env:RACEPREDICTOR_E2E_RUN_ID='release-readiness-20260924-online-final2'; npm run test:e2e:online` from `apps/web` | Online API fixture mode; Chromium; one worker; six redesigned routes, all-route axe, Plan/Calendar workflows and responsive matrix. | 44/44; exit 0; 0 failures, flaky attempts or skips. | `.local/e2e/release-readiness-20260924-online-final2/results/online.json` |
| `$env:RACEPREDICTOR_E2E_RUN_ID='release-readiness-20260924-auth-final2'; npm run test:e2e:auth` from `apps/web` | Owner-auth test configuration; Chromium; one worker; protected-route redirect, compact sign-in and fail-closed anonymous API checks. This is a fixture run, not deployed authentication evidence. | 3/3; exit 0; 0 failures or skips. | `.local/e2e/release-readiness-20260924-auth-final2/results/auth.json` |
| Focused `npx playwright test --config playwright.online.config.ts --grep 'online Plan selects|online Calendar saves a reasoned amendment|online Plan keeps a conflict|F05 scans expanded disclosures'` | Online fixtures; Chromium; one worker; modal heading levels and expanded Plan/Training/Settings disclosures. | 4/4; exit 0; 0 failures or skips. | `.local/e2e/release-readiness-20260924-a11y-dialogs-final/results/online.json` |
| `npm test --workspace @racepredictor/web`; `npm test --workspace @racepredictor/core`; `npm run typecheck --workspace @racepredictor/web`; `npm run lint --workspace @racepredictor/web` | Local unit/type checks; current uncommitted source and E2E tree at HEAD `0c5f5022b5b7cd600ade662b32907259f131c89c`. | 136/136 web tests and 90/90 core tests; typecheck and lint exit 0. Lint is the same TypeScript command as typecheck. | `.local/evidence/release-readiness-20260924-final/{web-unit,core-unit,web-typecheck,web-lint}.log` |
| Database local activity, import, coaching, analytics, pipeline, sync, Obsidian, direct activity-review and cloud test commands | Isolated local test fixtures; no live database configured. | 2 + 5 + 7 + 1 + 3 + 15 + 1 + 3 + 66 tests pass; every process exits 0. | `.local/evidence/release-readiness-20260924-final/db-*.log` and `summary.csv` |
| `npm run build --workspace @racepredictor/web` | Production Next.js build after all Playwright servers stopped; exit 0. | PASS. | `.local/evidence/release-readiness-20260924-final/web-build.log` |

The final F05 axe work corrected nested Plan-history session headings and dialog titles; activity content receives a deeper heading level inside Calendar dialogs while CSS preserves its prior typography. Schema-valid fixtures now render the Home prediction-settings disclosure and approved Plan history. Earlier failure screenshots/traces remain in their original run directories; final success reports are separate and contain no retries. The 2026-09-24 aggregate run exits cleanly in this Windows environment; the prior shutdown issue was an execution-permission limitation on cleaning up child processes, not a Playwright test failure.

**Release status: IN PROGRESS / not approved.** Browser-controlled 200% zoom, screen reader, switch control, software keyboard, unfamiliar-runner comprehension, real-provider OAuth cancellation/reconnection/persistence, deployed owner journey, anonymous deployed access controls, and deployment smoke remain unperformed. `DATABASE_URL` is unset; the separate `db:test:contracts` check is not included in these passing suites and must run only against an explicitly provisioned non-production database. No deployment or live-account change was performed.
